...
def process_smartrecruiters_companies(
    ...
    For example:
    - NBCUniversal: https://jobs.smartrecruiters.com/NBCUniversal3/
...
def smartrecruiters_company_urls(context: AssetExecutionContext) -> None:
    ...
    for file_path in csv_files:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    # Only include smartrecruiters companies
                    if platform.lower() == "smartrecruiters" or (row.get("platform", "").lower() == "smartrecruiters"):
                        company = {
                            "company_name": row["company_name"],
                            "company_industry": row["company_industry"],
                            "platform": "smartrecruiters"  # Ensure platform is set correctly
                        }
    ...
    if not all_companies:
        context.log.warning("No smartrecruiters company data was loaded from CSV files")
        # Return empty DataFrame with the correct structure
        return None

    context.log.info(f"Loaded {len(all_companies)} smartrecruiters companies from CSV files")
    ...
    context.log.info(f"{len(companies_to_process)} smartrecruiters companies remaining to process")
    ...
    for i in range(0, len(companies_to_process), batch_size):
        ...
        try:
            ...
            batch_results = process_smartrecruiters_companies(context, gemini, batch)
    ...
    context.log.info(f"Processed {total_companies} smartrecruiters companies")
    if total_companies > 0:
    ...
    # Create the smartrecruiters_company_urls table if it doesn't exist
    create_table_sql = f"""
    CREATE TABLE IF NOT EXISTS {dataset_name}.smartrecruiters_company_urls (
    ...
    context.log.info("Created or verified smartrecruiters_company_urls table")
    ...
    temp_table_name = f"{dataset_name}.smartrecruiters_company_urls_temp"
    ...
    try:
        ...
        transfer_sql = f"""
        INSERT INTO {dataset_name}.smartrecruiters_company_urls
        ...
        LEFT JOIN {dataset_name}.smartrecruiters_company_urls m
        ...
        count_sql = f"SELECT COUNT(*) FROM {dataset_name}.smartrecruiters_company_urls"
        query_job = client.query(count_sql)
        count_result = list(query_job.result())
        count = count_result[0][0]
        context.log.info(f"Total records in smartrecruiters_company_urls table: {count}")
        ...
        context.add_output_metadata({
            ...
            "bigquery_table": MetadataValue.text(f"{dataset_name}.smartrecruiters_company_urls"),
