...
def retry_failed_greenhouse_company_urls(context: AssetExecutionContext, gemini: GeminiResource) -> pd.DataFrame:
    ...
    # Path for tracking companies that switched ATS
    url_discovery_checkpoint = checkpoint_dir / "url_discovery_checkpoint.csv"

    # Pull companies with url_verified=FALSE from the database
    context.log.info("Querying for Greenhouse companies with unverified URLs")

    try:
        # SQL to fetch companies with unverified URLs
        sql = """
        SELECT company_name, company_industry, platform
        FROM public.greenhouse_company_urls
    ...
    if not failed_companies:
        context.log.info("No unverified Greenhouse companies found")
        return pd.DataFrame(columns=["company_name", "company_industry", "platform", "ats_url", "career_url", "url_verified"])

    # Process companies individually with additional safeguards
    results = []
    ats_switched_companies = []

    # Load existing ATS switched companies if the file exists
    if url_discovery_checkpoint.exists():
        try:
            switched_df = pd.read_csv(url_discovery_checkpoint)
            existing_switched = switched_df.to_dict("records")
            context.log.info(f"Loaded {len(existing_switched)} existing ATS switched companies")
            ats_switched_companies.extend(existing_switched)
        except Exception as e:
    ...
    for company in failed_companies:
        ...
        try:
            ...
            # If URLs were verified successfully, add to results
            if validated_results and validated_results[0].get("url_verified", False):
            ...
            for alt_platform in alternative_platforms:
                ...
                alt_result = {
                    ...
                    "career_url": alt_urls.get("career_url"),
                    "url_verified": False
                }

                # Validate URLs for alternative platform
                alt_validated = validate_urls(context, [alt_result])
                ...
                if alt_validated and alt_validated[0].get("url_verified", False):
                    ...
                    switched_entry = alt_validated[0].copy()
                    switched_entry["original_platform"] = "greenhouse"
                    switched_entry["new_platform"] = alt_platform
