...
def lever_company_urls(context: AssetExecutionContext) -> None:
    ...
    # Create the lever_company_urls table if it doesn't exist
    create_table_sql = f"""
    CREATE TABLE IF NOT EXISTS {dataset_name}.lever_company_urls (
    ...
    context.log.info("Created or verified lever_company_urls table")
    ...
    temp_table_name = f"{dataset_name}.lever_company_urls_temp"
    ...
    try:
        ...
        transfer_sql = f"""
        INSERT INTO {dataset_name}.lever_company_urls
        ...
        LEFT JOIN {dataset_name}.lever_company_urls m
        ...
        count_sql = f"SELECT COUNT(*) FROM {dataset_name}.lever_company_urls"
        query_job = client.query(count_sql)
        count_result = list(query_job.result())
        count = count_result[0][0]
        context.log.info(f"Total records in lever_company_urls table: {count}")
        ...
        context.add_output_metadata({
            ...
            "bigquery_table": MetadataValue.text(f"{dataset_name}.lever_company_urls"),
...
def retry_failed_lever_company_urls(context: AssetExecutionContext) -> None:
    """
    Retries finding career URLs for Lever companies that failed verification.
    Uses the generic retry function with Lever-specific configuration.
